...
export async function processSignatures({
  ...
  let callOptions = {};
    ...
    try {
      ...
      callOptions = {
        maxFeePerGas: parseUnits((Math.min(parseInt(gasPriceAPI.result.FastGasPrice),150)).toString(), 'gwei'),
        maxPriorityFeePerGas: parseUnits((Math.min(parseInt(gasPriceAPI.result.FastGasPrice),150)).toString(), 'gwei'),
      };
    } catch {
